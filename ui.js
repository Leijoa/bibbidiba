...
export function showToast(msg, callback) {
    ...
    const spacing = 10;
    let currentY = window.innerHeight / 2 - 50;
...
export function updateHeaderUI(player, stage) {
    if (stage.level < ENEMY_DB.length) {
        ...
        el.stageInfo.setAttribute('data-i18n-args', `∞ ${n}-${m}`);
    }
...
// --- 牌型結算渲染 ---
// ⚠️ 這裡有加上反引號 (`) 和分號 (;) 修正了先前的 Syntax Error!
...
export function renderShopItems(shopItems, player) {
    const renderItem = (r, idx) => {
        const btnClass = "bg-yellow-600 hover:bg-yellow-500 text-white active:scale-95 shadow-md border-b-4 border-yellow-800 active:border-b-0 active:translate-y-1";
        const style = RARITY[r.rarity];
        ...
        const rName = r.id.startsWith('cons_') ? i18n.t(`consumables.${r.id}.name`) : (i18n.t(`relics.${r.id}.name`) || r.name);
        const rDesc = r.id.startsWith('cons_') ? i18n.t(`consumables.${r.id}.desc`) : (i18n.t(`relics.${r.id}.desc`) || r.desc);
        const rarityLabel = i18n.t(`messages.rarity_${r.rarity}`) || style.label;
        const fusionHint = i18n.t('ui.shop_fusion_hint') || '可融合';
        const selectBtnText = i18n.t('messages.shop_select');
        
        const fusionSpan = isFusionMaterial ? `<span onclick="window.showFusionInfo('${fusionResultId}')" class="text-sm md:text-base cursor-pointer px-1.5 py-0.5 rounded bg-cyan-900/60 text-cyan-300 border border-cyan-500 font-black shadow-[0_0_8px_rgba(34,211,238,0.4)] animate-pulse hover:bg-cyan-800 hover:scale-105 active:scale-95 transition-all">✨ ${fusionHint}</span>` : '';
        
        return `<div class="bg-slate-800 p-3 rounded-xl border border-slate-600 flex flex-col justify-between relative overflow-hidden">
                ...
                <div class="flex flex-col gap-1 mb-2">
                    <div class="flex justify-between items-start">
                        ...
                        <div class="flex flex-col items-end gap-1">
                            <span class="text-[9px] md:text-xs px-1.5 py-0.5 rounded ${style.bg} ${style.color} border ${style.border} font-bold">${rarityLabel}</span>
                            ${fusionSpan}
                        </div>
                    </div>
                </div>
    ...
    el.shopItemsContainer.innerHTML = shopItems.map(renderItem).join('');
...
export function showFusionReplaceModal(currentFusions, newFusionId, callback) {
    ...
    allRelics.forEach((id, index) => {
        ...
        const rName = i18n.t(`relics.${relic.id}.name`) || relic.name;
        const rDesc = i18n.t(`relics.${relic.id}.desc`) || relic.desc;
        ...
        if (FUSION_RECIPES[id]) {
            ...
            const mat1Name = mat1 ? (i18n.t(`relics.${mat1.id}.name`) || mat1.name) : FUSION_RECIPES[id].mat1;
            const mat2Name = mat2 ? (i18n.t(`relics.${mat2.id}.name`) || mat2.name) : FUSION_RECIPES[id].mat2;
            
            const returnMatText = i18n.t('ui.fusion_materials') || '退回素材：';
        ...
        const newFusionText = i18n.t('ui.fusion_new_item') || '本次合成';
        const discardBtnText = i18n.t('ui.fusion_discard_btn') || '捨棄並分解';
        
        let borderColorClass = isNew ? 'border-amber-400 shadow-[0_0_15px_rgba(251,191,36,0.3)]' : 'border-slate-600';
        let ribbonHtml = isNew ? `<div class="absolute top-2 -right-8 bg-amber-500 text-slate-900 text-[10px] font-black w-32 text-center py-1 rotate-45 z-20 shadow-md">${newFusionText}</div>` : '';
        let rarityLabel = i18n.t(`messages.rarity_${relic.rarity}`) || style.label;

        html += `
        <div class="bg-slate-900/80 border-2 ${borderColorClass} rounded-xl p-3 md:p-4 flex flex-col justify-between h-full relative overflow-hidden">
            ${ribbonHtml}
            <div class="relative z-10">
                <div class="flex justify-between items-start mb-2 mt-2">
                    <h3 class="text-base md:text-lg font-black ${style.color}">${rName}</h3>
                    <span class="text-[10px] md:text-xs px-2 py-0.5 rounded ${style.bg} ${style.color} border ${style.border} font-bold z-10 relative mt-1 mr-4">${rarityLabel}</span>
                </div>
                <p class="text-xs md:text-sm text-slate-300 font-bold mb-3">${rDesc}</p>
                ${materialsHtml}
            </div>

            <button onclick="window.selectFusionDiscard('${id}')" class="w-full mt-4 bg-red-950/80 hover:bg-red-900 border border-red-800 hover:border-red-500 text-red-400 hover:text-white font-black py-2.5 rounded-lg transition-all active:scale-95 text-sm md:text-base relative z-10">
                ${discardBtnText}
            </button>
        </div>
        `;
...
export function renderHistoryModal(records, metaData) {
    ...
    if (metaData && metaData.stats) {
        ...
        const pbTitle = i18n.t('ui.pb_title') || '🏆 個人最佳紀錄';
        const pbHighestDmg = i18n.t('ui.pb_highest_dmg') || '最高傷害';
        const pbHighestMulti = i18n.t('ui.pb_highest_multi') || '最高倍率';
        const pbHighestInfinite = i18n.t('ui.pb_highest_infinite') || '最高無限層數';
        const highestMultiStr = metaData.stats.highestMulti.toLocaleString(undefined, {minimumFractionDigits: 1, maximumFractionDigits: 1});
        const infiniteLevelStr = metaData.stats.highestInfiniteLevel > 0 ? metaData.stats.highestInfiniteLevel : '-';
        const relicsHtml = metaData.stats.highestDamageRelics.map(r => {
            const icon = window.RELIC_DB[r]?.icon || 'img/relic_placeholder.png';
            const name = window.RELIC_DB[r]?.name || '';
            return `<img src="${icon}" class="w-6 h-6 rounded-md border border-slate-600 shadow-sm" title="${name}">`;
        }).join('');

        pbHtml = `<div class="bg-amber-900/40 border border-amber-600/50 p-4 rounded-xl mb-6 shadow-inner">
            <h3 class="text-lg font-black text-amber-400 mb-3 flex items-center gap-2">${pbTitle}</h3>
            <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                <div>
                    <div class="text-xs text-amber-200/70 font-bold">${pbHighestDmg}</div>
                ...
                <div>
                    <div class="text-xs text-amber-200/70 font-bold">${pbHighestMulti}</div>
                    <div class="text-xl font-black text-emerald-400">x${highestMultiStr}</div>
                </div>
                <div>
                    <div class="text-xs text-amber-200/70 font-bold">${pbHighestInfinite}</div>
                    <div class="text-xl font-black text-purple-400">${infiniteLevelStr}</div>
                </div>
            </div>
    ...
    if (!records || records.length === 0) {
        const historyEmpty = i18n.t('messages.history_empty');
        el.historyContent.innerHTML = pbHtml + `<div class="text-center text-slate-500 py-6 font-bold">${historyEmpty}</div>`;
        return;
    }
    
    el.historyContent.innerHTML = pbHtml + records.map((r, i) => {
        const resultColor = r.win ? "text-amber-400" : "text-red-400";
        const resultText = r.stageName || (r.win ? "勝利" : "失敗");
        const dateObj = new Date(r.date);
        const dateStr = dateObj.toLocaleDateString() + " " + dateObj.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'});
        
        const relicHtml = (r.relics && r.relics.length > 0) ? r.relics.map(id => {
            let relicDef = RELIC_DB.find(x => x.id === id);
            if (!relicDef) return '';
            const rName = id.startsWith('cons_') ? i18n.t(`consumables.${id}.name`) : (i18n.t(`relics.${id}.name`) || relicDef.name);
        ...
        return `<div class="bg-slate-800 p-3 rounded-lg border border-slate-700 flex flex-col gap-1 relative overflow-hidden">
...
export function renderEndGameStats(highestDamage, highestDamageCombo, relics) {
    ...
    const dmgLabel = i18n.t('messages.history_dmg_label');
    const relicsLabel = i18n.t('messages.history_relics_label');
    const noneLabel = i18n.t('messages.none');
    
    el.endStats.innerHTML = `<div class="bg-slate-900/80 p-3 rounded-lg border border-slate-700/50 w-full max-w-sm mx-auto shadow-inner text-left">
        <div class="mb-2 border-b border-slate-700/50 pb-2">
            <div class="text-xs text-slate-400 mb-1">${dmgLabel}</div>
            <div class="text-2xl md:text-3xl font-black text-white">${Number(highestDamage).toLocaleString()}</div>
            <div class="text-sm font-bold text-blue-300 mt-1">${highestDamageCombo || noneLabel}</div>
        </div>
        <div>
            <div class="text-xs text-slate-400 mb-1.5">${relicsLabel}</div>
...
export function renderCollectionModal(tab) {
    ...
    if (tab === 'hands') {
        ...
        groups.forEach(g => {
            const groupTitle = i18n.t(g.titleKey);
            html += `<h3 class="text-base md:text-lg font-black text-slate-300 mt-2 mb-1 border-b border-slate-700 pb-1">${groupTitle}</h3>`;
            html += `<div class="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-3">`;
            RULE_DB[g.key].forEach((rule, rIdx) => {
                const unlocked = coll.hands.includes(rule.name);
                let letter = g.key.replace('group', '').toLowerCase();
                const ruleName = i18n.t(`rules.rule_${letter}${rIdx}.name`) || rule.name;
                const ruleDesc = i18n.t(`rules.rule_${letter}${rIdx}.desc`) || rule.desc;

                const nameStr = unlocked ? `${ruleName} <span class="text-emerald-400 text-xs ml-1">✅</span>` : `???`;
                const lockedLabel = i18n.t('locked');
                const descStr = unlocked ? ruleDesc : lockedLabel;
                const opacity = unlocked ? 'opacity-100' : 'opacity-50 grayscale';
                let rStyle = RARITY[rule.rarity] || RARITY[1];
                let nameColor = unlocked ? rStyle.color : 'text-slate-200';
                html += `<div class="flex justify-between items-center bg-slate-900/50 p-2.5 rounded-lg border border-slate-700 ${opacity}">
                    <div>
                        <div class="flex items-center gap-2">
        ...
        RELIC_DB.forEach(r => {
            ...
            if (unlocked) {
                ...
                if (r.rarity === 5 && FUSION_RECIPES[r.id]) {
                    ...
                    let mat1Def = RELIC_DB.find(x => x.id === mat1Id);
                    let mat2Def = RELIC_DB.find(x => x.id === mat2Id);
                    const mat1Name = mat1Def ? (i18n.t(`relics.${mat1Id}.name`) || mat1Def.name) : mat1Id;
                    const mat2Name = mat2Def ? (i18n.t(`relics.${mat2Id}.name`) || mat2Def.name) : mat2Id;
                    const fusionMsg = i18n.t('ui.fusion_text_short', mat1Name, mat2Name);
                    fusionText = `<p class="text-xs text-amber-300 mt-1 border-t border-slate-600 pt-1">${fusionMsg}</p>`;
                }
                html += `<div class="bg-slate-800 p-2 rounded-xl border border-slate-600 flex flex-col justify-between relative overflow-hidden">
                ...
                const lockedLabel = i18n.t('locked_relic');
                html += `<div class="bg-slate-900 p-2 rounded-xl border border-slate-700 flex flex-col justify-between relative overflow-hidden opacity-50">
                    <div class="flex justify-between items-start mb-1">
                        <h3 class="text-sm md:text-base font-black text-slate-500">???</h3>
                    </div>
                    <p class="text-xs md:text-sm text-slate-600 font-bold">${lockedLabel}</p>
                </div>`;
            }
        });
        html += `</div>`;
        ...
        SHACKLE_DB.forEach(s => {
            ...
            if (unlocked) {
                ...
                const sDesc = i18n.t(`shackles.${s.id}.desc`) || s.desc;
                html += `<div class="bg-slate-800 p-2 rounded-xl border border-slate-600 flex flex-col justify-between relative overflow-hidden">
                    <div class="flex justify-between items-start mb-1">
                ...
                const lockedLabel = i18n.t('locked_shackle');
                html += `<div class="bg-slate-900 p-2 rounded-xl border border-slate-700 flex flex-col justify-between relative overflow-hidden opacity-50">
                    <div class="flex justify-between items-start mb-1">
                        <h3 class="text-sm md:text-base font-black text-slate-500">???</h3>
                    </div>
...
export function renderSoulsModal(metaData) {
    ...
    el.soulsContent.innerHTML = SOULS_UPG_DEFS.map(u => {
        const currentLv = metaData.upgrades[u.id] || 0;
        const isMax = currentLv >= u.max;
        const currentCost = u.cost(currentLv);
        const canAfford = metaData.souls >= currentCost;

        const uName = i18n.t(`souls.${u.id}.name`) || u.name;
        const uDesc = i18n.t(`souls.${u.id}.desc`) || u.desc;

        const dots = Array(u.max).fill().map((_, i) => i < currentLv ? '🟢' : '⚫').join(' ');
        const maxedBtn = i18n.t('souls.maxed');
        const costLabel = i18n.t('souls.cost', currentCost);
        const btnClass = canAfford ? 'bg-indigo-600 hover:bg-indigo-500 text-white shadow-[0_0_10px_rgba(79,70,229,0.5)]' : 'bg-slate-700 text-slate-500 cursor-not-allowed';
        let btnHtml = isMax
            ? `<button disabled class="bg-slate-700 text-slate-500 font-bold py-2 px-4 rounded-lg cursor-not-allowed">${maxedBtn}</button>`
            : `<button onclick="window.buySoulUpgrade('${u.id}', ${currentCost})" class="${btnClass} font-black py-2 px-4 rounded-lg transition-transform active:scale-95" ${canAfford ? '' : 'disabled'}>${costLabel}</button>`;

        return `<div class="bg-slate-900/50 border border-indigo-900/50 p-3 rounded-xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
            <div>
    ...
    const resetSoulsBtn = i18n.t('ui.reset_souls') || '重置靈魂 (退還所有花費)';
    el.soulsContent.innerHTML += `<div class="mt-4 pt-4 border-t border-slate-700/50">
            <button onclick="window.resetSouls()" class="w-full bg-red-600/80 hover:bg-red-500/80 text-white py-2 rounded font-bold transition-colors">${resetSoulsBtn}</button>
        </div>`;
}
