...
export function calculateEngineScore(dice, playerRelics, rollsLeft) {
    ...
    if (bFull) tagB = { name: '大滿貫', multi: playerRelics.includes('blue') ? 24.0 : 8.0, used: bFull };
    ...
    else if (counts[1]+counts[3]+counts[5]+counts[7] === 8 || counts[2]+counts[4]+counts[6]+counts[8] === 8) tagD = { name: '絕對秩序', multi: 8.0, used: dice.map(d=>d.val) };
